...
class TestAIGenerator:
    ...
    def create_tool_use_block(
        self, tool_name="search_course_content", tool_id="tool_1", input_data=None
    ):
    ...
    def test_api_error_during_sequential_calls(self, mock_anthropic):
        ...
        assert (
            "Error during tool execution round 1: API Error during second call"
            in result
        )
        assert mock_client.messages.create.call_count == 2
        assert self.mock_tool_manager.execute_tool.call_count == 1
    ...
    def test_message_context_building(self, mock_anthropic):
        ...
        # Execute
        result = ai_gen.generate_response(
            "Initial query", tools=self.tools, tool_manager=self.mock_tool_manager
        )
